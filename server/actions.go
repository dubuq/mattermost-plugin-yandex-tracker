...
func (p *Plugin) handleCollapse(w http.ResponseWriter, r *http.Request) {
	p.toggleCollapse(w, r, true)
}

func (p *Plugin) handleExpand(w http.ResponseWriter, r *http.Request) {
	p.toggleCollapse(w, r, false)
}

func (p *Plugin) toggleCollapse(w http.ResponseWriter, r *http.Request, collapse bool) {
	var req model.PostActionIntegrationRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "bad request", http.StatusBadRequest)
		return
...
func (p *Plugin) handleChangeStatus(w http.ResponseWriter, r *http.Request) {
	var req model.PostActionIntegrationRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "bad request", http.StatusBadRequest)
		return
...
func (p *Plugin) handleTransition(w http.ResponseWriter, r *http.Request) {
	...
	if len(requiredFields) > 0 {
		...
		if err := p.store.SetPendingTransition(req.UserId, PendingTransition{
			IssueKey:       issueKey,
			PostID:         postID,
...
func (p *Plugin) handleFillFields(w http.ResponseWriter, r *http.Request) {
	var req model.PostActionIntegrationRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "bad request", http.StatusBadRequest)
		return
	...
	pending, err := p.store.GetPendingTransition(req.UserId)
	if err != nil || pending == nil {
		p.API.LogError("fill-fields: no pending transition", "userID", req.UserId)
		writeActionResponse(w, nil)
		return
	}
...
func (p *Plugin) handleFillFieldsSubmit(w http.ResponseWriter, r *http.Request) {
	var req model.SubmitDialogRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "bad request", http.StatusBadRequest)
		return
	...
	if req.Cancelled {
		p.store.DeletePendingTransition(req.UserId)
		writeSubmitDialogResponse(w, "")
		return
	}
...
func (p *Plugin) handleAddComment(w http.ResponseWriter, r *http.Request) {
	...
	if req.PostID == "" || req.IssueKey == "" {
		respondJSON(w, map[string]interface{}{"ok": false, "error": "post_id and issue_key are required"})
		return
	}
